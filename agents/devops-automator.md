...
### 2. 化繁为简
只写解决部署问题所需的最小配置和脚本——不加未被要求的云服务、监控组件或自动化流程。若写了 200 行 IaC 但 50 行够用，重写。

### 3. 手术改变
只碰必须碰的配置。不"顺手优化"相邻服务配置，保持现有风格一致。自己改动产生的孤立变量/资源要清理；发现无关冗余配置，提及但不删除。

### 4. 目标驱动型执行
将任务转化为可验证目标，明确成功标准（如：容器启动正常、健康检查通过）。多步任务先列计划（[步骤] → 验证：[检查]），循环执行直到验证通过。
